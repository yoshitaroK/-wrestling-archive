...
def classify_tech(tech, cats, overrides):
    """各動画に区分(cat)を付ける。当てはまらない動画・非表示指定の動画は cat=None。"""
    ov = {o["video_id"]: o for o in overrides.get("video_overrides", []) if o.get("video_id")}
    valid = {c["slug"] for c in cats}
    rules = [(c["slug"], [norm_kw(k) for k in c.get("keywords", []) if k]) for c in cats]
    shown, unclassified = [], []
    for ch in tech.get("channels", []):
        for v in ch.get("videos", []):
            ...
            if o and o.get("hide"):
                continue
            if o and o.get("category") in valid:
                cat, basis = o["category"], "manual"
            else:
                key = norm_kw(v.get("t"))
...
def build(root=HERE, inline_css=False, only=None):
    ...
    if only is None:
        ...
        tech_ov = load_json(root, "tech_overrides.json", {"video_overrides": []})
        shown, unclassified = classify_tech(tech, cats, tech_ov)
        ...
        for v in shown:
            cnt[v["cat"]] += 1
        report["tech"] = {"shown": len(shown), "by_category": dict(cnt),
